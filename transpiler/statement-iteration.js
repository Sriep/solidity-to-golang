...
module.exports = {
    ...
    codeWhile: function(node, history, parent, localHistory, depth, declarations) {
    ...
    codeDoWhile: function (node, history, parent, localHistory, depth, declarations) {
    ...
    codeFor: function(node, history, parent, localHistory, depth, declarations) {
        assert(node && node.type === "ForStatement");
        const block = require("./block.js");
        let goCode = "\t".repeat(depth);
        goCode += "for ";
        if (node.init)
            goCode += expression.codeExpression(node.init, history, parent, localHistory, depth, declarations);
        goCode += " ; ";
        if (node.test)
            goCode += expression.codeExpression(node.test, history, parent, localHistory, depth, declarations);
        goCode += " ; ";
        if (node.update)
            goCode += expression.codeExpression(node.update, history, parent, localHistory, depth, declarations);
        goCode += block.codeStatement(node.body, history, parent, localHistory, depth+1, declarations);
        return goCode;
    }
