...
module.exports = {
    ...
    code: function (node, history, parent, localHistory, depth) {
        ...
        goCode += block.codeStatement(node.consequent, history, parent, localHistory, depth+1);
        if (node.alternate) {
            if (goCode[goCode.length -1] == "\n")
                goCode = goCode.slice(0, -1);
            goCode += " else ";
            goCode += block.codeStatement(node.alternate, history, parent, localHistory, depth+1);
        }
        return goCode;
    }
};
