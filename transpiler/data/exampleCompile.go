...
// Above is same for all .sol file compilations
// ****************************************************************************************
// Below varies depending on input .sol file

type Base_pub interface {}
type Base interface {}
type Base_int interface {}
type Base_pri interface {}
type Base_S struct {
	Contract
	this *Base_S
}
func NewBase(_msg msg_) (*Base_S) {
	msg = _msg
	p := new(Base_S)
	p.this = p
	p.create("xBase", 0)
...
type Derived_pub interface {
	Base_pub
	PubF(_value uint) (uint)
	PubSV() (uint)
}
...
type Derived_int interface {
	Base_int
	Derived_pub
	__intF(_value uint)
}
type Derived_pri interface {
	Base_pri
	Derived_int
	__priF(_value uint)
}
type Derived_S struct {
	Contract
	this *Derived_S
}
func NewDerived(_msg msg_) (*Derived_S) {
	msg = _msg
	p := new(Derived_S)
	p.this = p
	p.create("xBase", 0)
...
// Public functions an  d state variables
func (s *Derived_S) PubF(_value uint) (uint) {
	s.set("pubSV", _value)
	v := s.get("pubSV").(uint)
	return v
}
func (s *Derived_S) PubSV() (uint){
	v := s.get("pubSV").(uint)
	return v
}

// External functions
func (s *Derived_S) ExtF(_value uint, _msg msg_){
	msg = _msg
	s.set("pubSV", _value)
}

// Internal functions and state variables
func (s *Derived_S) __intF(_value uint) (uint) {
	s.set("intSV", _value)
	v := s.get( "pubSV").(uint)
	return v
}
...
func (s *Derived_S) __priF(_value uint) (uint) {
	s.set("priSV", _value)
	v := s.get("pubSV").(uint)
	return v
}
...
func main() {
	...
	der1 := NewDerived(msg_{})
	der1.set("myv", 72)
	avar := 2*der1.get( "myv").(int)+1
	fmt.Println(der1)
	fmt.Println(avar)
	fmt.Println("Hi")
}