...
func NewInbox(initialMessage string) (*_Inbox_st) {
	...
	p.this = p
	p.createStorage()
	p.set("message",  new(string))
	p.constructor__Inbox(initialMessage )
	return p
}
...
func (this _Inbox_st) SetMessage(newMessage string) {
